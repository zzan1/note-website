<p style="text-align: center; font-size: 32px"><strong>JavaScript 高级程序设计的笔记</strong></p>
...
引用类型值就是保存在内存中的对象，**变量名作为一个指针**指向这个内存中的对象。object 就是引用类型的值。
...
除了一些特殊的值 undefined, NaN 和 引用类型的构造函数等。错误对象的构造函数也是它的属性。
